################--------------------------------------------
## Supplementary Figure S21. CosMx coordinate matching and state remapping overview
##
## Purpose:
##   1) Load CosMx coordinate matching and RNA-defined state remapping table
##   2) Generate and export Supplementary Figure S21 (Panel A coordinate matching overview, Panel B state remapping overview)
##
## Output:
##   D:/ICB_resistance_project/results/figures/supplementary/
##     - Supplementary Figure S21. CosMx coordinate matching and state remapping overview.png
##     - Supplementary Figure S21. CosMx coordinate matching and state remapping overview.jpg
##     - Supplementary Figure S21. CosMx coordinate matching and state remapping overview.pdf
################--------------------------------------------
...
dpi_out <- 300
base_filename <- "Supplementary Figure S21. CosMx coordinate matching and state remapping overview"
...
theme_s21 <- theme_bw(base_size = 11) +
  theme(
    plot.title = element_text(size = 14.5, face = "bold", hjust = 0, color = "black"),
    plot.subtitle = element_text(size = 9.8, hjust = 0, color = "black"),
    ...
    legend.text = element_text(size = 8.4, color = "black", lineheight = 0.88),
...
pA <- ggplot(plot_df, aes(x = x, y = y)) +
  ...
  labs(
    ...
    subtitle = paste0("Matched CosMx cells with polygon-derived global coordinates; y-axis inverted for display; n = ", n_cells_label),
    x = "Spatial x",
    y = "Spatial y"
  ) +
  theme_s21 +
...
pB <- ggplot(plot_df, aes(x = x, y = y, color = state)) +
  ...
  labs(
    ...
    subtitle = "Unit: CosMx cell; polygon-derived global coordinates; y-axis inverted for display; localization overview, not functional validation",
    x = "Spatial x",
    y = "Spatial y"
  ) +
  theme_s21 +
...
safe_ggsave(out_pdf, supp_s21, width = fig_width, height = fig_height, dpi = dpi_out, device = cairo_pdf)
...
audit_file <- file.path(fig_dir, "Supplementary_Figure_S21_CosMx_coordinate_matching_state_remapping_audit.csv")
...
message("Supplementary Figure S21 script finished successfully.")