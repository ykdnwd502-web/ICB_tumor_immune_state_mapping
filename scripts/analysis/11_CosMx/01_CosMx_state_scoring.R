...
## v1.1 clean public rerun entry.
##
## PUBLIC SEQUENTIAL ROLE
##   raw Run5611_MK3 expression matrix + locked four-state GMT
##     -> CosMx cell-level state-score table
##
## Deterministic public-path edits only:
##   * explicit raw expression file
##   * explicit locked four-state GMT
## Scientific/statistical scoring logic is unchanged from the frozen Step14B v4 authority.
############################################################

# ============================================================
# Step 14B v4. Rebuild CosMx cell-level state scores for IF/mIF merge
# Composite-ID version: fov + cell_ID
# ------------------------------------------------------------
# Why v4:
#   CosMx cell_ID is not globally unique across FOVs. The correct key is
#   fov + cell_ID. This script outputs .merge_id = paste(fov, cell_ID).
#
# Run first:
#   source("D:/ICB_resistance_project/scripts/14B_rebuild_CosMx_state_scores_for_IF_mIF_v4_COMPOSITE_ID.R")
#
# Then run:
#   source("D:/ICB_resistance_project/scripts/14A_rebuild_IF_mIF_table_and_run_MINIMAL_v4_COMPOSITE_ID.R")
# ============================================================
...
############################################################
## v1.1 PUBLIC CLEAN-RERUN RESET
## Remove only CURRENT generated CosMx downstream outputs.
## Raw data and FROZEN_BACKUP are never touched.
############################################################

.cosmx_generated_dirs <- c(
  file.path(project_dir, "results", "tables", "CosMx_spatial_validation"),
  file.path(project_dir, "results", "tables", "CosMx_dual_high_QC_sensitivity"),
  file.path(project_dir, "results", "tables", "CosMx_dual_high_QC_sensitivity_with_metadata_v2"),
  file.path(project_dir, "results", "tables", "CosMx_dual_high_QC_sensitivity_with_metadata_v3"),
  file.path(project_dir, "results", "tables", "CosMx_metadata_QC"),

  file.path(project_dir, "results", "figures", "CosMx_spatial_validation"),
  file.path(project_dir, "results", "figures", "CosMx_dual_high_QC_sensitivity"),
  file.path(project_dir, "results", "figures", "CosMx_dual_high_QC_sensitivity_with_metadata_v2"),
  file.path(project_dir, "results", "figures", "CosMx_dual_high_QC_sensitivity_with_metadata_v3"),

  file.path(project_dir, "results", "diagnostics", "11_CosMx"),
  file.path(project_dir, "results", "canonical", "S21_S22_CosMx_v1.0")
)

for (.d in .cosmx_generated_dirs) {
  if (dir.exists(.d)) {
    message("Removing stale/current CosMx output: ", .d)
    unlink(.d, recursive = TRUE, force = TRUE)
  }
}

.step14b_dir <- file.path(
  project_dir, "results", "tables", "IF_mIF_validation"
)

if (dir.exists(.step14b_dir)) {
  .step14b_files <- list.files(
    .step14b_dir,
    pattern = "^Step14B_.*CosMx|^Step14B_v4_",
    full.names = TRUE
  )
  if (length(.step14b_files)) {
    unlink(.step14b_files, force = TRUE)
  }
}
...
safe_write_csv(expr_gene_inventory, file.path(out_table_dir, "CosMx_expression_gene_inventory.csv"))
...
safe_write_csv(gs_audit_df, file.path(out_table_dir, "CosMx_gene_set_candidate_overlap_audit.csv"))

usable <- gs_audit_df %>% filter(selected_candidate)
if (nrow(usable) == 0) {
  stop("No usable gene-set file had sufficient overlap with the CosMx gene panel. Inspect CosMx_gene_set_candidate_overlap_audit.csv.")
}
...
safe_write_csv(gene_sets, file.path(out_table_dir, "CosMx_state_gene_sets_used.csv"))
safe_write_csv(overlap_audit, file.path(out_table_dir, "CosMx_state_score_gene_overlap_audit.csv"))
...
out_score <- file.path(out_table_dir, "CosMx_cell_state_scores.csv")
...
safe_write_csv(state_summary, file.path(out_table_dir, "CosMx_cell_state_score_summary.csv"))

message("Step 14B v4 finished.")
message("State-score table saved to: ", out_score)
message("Next: run 14A v4.")


## Public sequential end gate
.step01_out <- file.path(
  out_table_dir,
...
cat("\n============================================================\n")
cat("STEP 01 PASS — CosMx raw expression -> state scores\n")
cat("State-score rows: ", .step01_n, "\n", sep = "")
cat("Output: ", .step01_out, "\n", sep = "")
cat("============================================================\n")
