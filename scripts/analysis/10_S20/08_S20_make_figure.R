...
dpi_out <- 300
out_prefix <- "Supplementary_Figure_S20_external_spatial_recurrence"

## Manual overrides. Leave blank to use the canonical source-locked S20 plot inputs.
manual_panelA_file <- ""
manual_panelB_file <- ""
manual_panelC_file <- ""
manual_panelD_file <- ""
...
## Project-wide public-figure style lock.
## Same state colors / sans typography / sizing convention as the bulk and
## external-validation figures already frozen for GitHub.
...
primary_state_colors <- state_colors_public[c(
  "myeloid–Treg immunosuppressive",
  "tumor-dedifferentiation/stromal-remodeling"
)]

theme_s20 <- theme_bw(base_size = 12.5, base_family = "sans") +
  theme(
    plot.title = element_text(size = 15, face = "bold", hjust = 0.5, color = "black", family = "sans"),
    plot.subtitle = element_text(size = 11.5, hjust = 0.5, color = "black", family = "sans"),
    plot.tag = element_text(size = 16, face = "bold", color = "black", family = "sans"),

    ## Axis titles are intentionally asymmetric:
    ## long vertical y-axis titles are slightly smaller for multi-panel readability.
    axis.title = element_text(size = 12.5, face = "bold", color = "black", family = "sans"),
    axis.title.y = element_text(size = 11.5, face = "bold", color = "black", family = "sans"),
    axis.title.x = element_text(size = 12.5, face = "bold", color = "black", family = "sans"),

    axis.text = element_text(size = 11, color = "black", family = "sans"),
    ...
    legend.title = element_text(size = 12.5, face = "bold", color = "black", family = "sans"),
    legend.text = element_text(size = 11, color = "black", family = "sans"),
...
############################################################
## 2. Locate hard-locked S28 input files
############################################################

panelA_file <- if (nzchar(manual_panelA_file)) manual_panelA_file else first_existing(
  c(
    file.path(s28_dir, "S20_plot_input_overview.csv"),
    file.path(s28_dir, "S20_dataset_overview.csv")
  ),
  "Panel A overview"
)
...
panelB_file <- if (nzchar(manual_panelB_file)) manual_panelB_file else first_existing(
  c(
    file.path(s28_dir, "S20_combined_gene_coverage_summary.csv"),
    file.path(s28_dir, "S20_primary_and_composition_gene_coverage.csv"),
    file.path(s28_dir, "S20_plot_input_coverage.csv")
  ),
  "Panel B coverage"
)
...
panelC_file <- if (nzchar(manual_panelC_file)) manual_panelC_file else first_existing(
  c(
    file.path(s28_dir, "S20_plot_input_spearman.csv"),
    file.path(s28_dir, "S20_primary_pair_focused_summary.csv"),
    file.path(s28_dir, "S20_combined_key_readout.csv")
  ),
  "Panel C Spearman"
)
...
panelD_file <- if (nzchar(manual_panelD_file)) manual_panelD_file else first_existing(
  c(
    file.path(s28_dir, "S20_plot_input_dual_high_OR.csv"),
    file.path(s28_dir, "S20_primary_pair_focused_summary.csv"),
    file.path(s28_dir, "S20_combined_key_readout.csv")
  ),
...
panelE_file <- if (nzchar(manual_panelE_file)) manual_panelE_file else first_existing(
  c(
    file.path(s28_dir, "S20_plot_input_moran.csv"),
    file.path(s28_dir, "S20_primary_pair_focused_summary.csv"),
    file.path(s28_dir, "S20_combined_key_readout.csv")
  ),
  "Panel E Moran"
...
message("Panel A file: ", panelA_file)
message("Panel A columns: ", paste(colnames(panelA_raw), collapse = ", "))
message("Panel B file: ", panelB_file)
message("Panel B columns: ", paste(colnames(panelB_raw), collapse = ", "))
message("Panel C file: ", panelC_file)
message("Panel C columns: ", paste(colnames(panelC_raw), collapse = ", "))
message("Panel D file: ", panelD_file)
message("Panel D columns: ", paste(colnames(panelD_raw), collapse = ", "))
message("Panel E file: ", panelE_file)
message("Panel E columns: ", paste(colnames(panelE_raw), collapse = ", "))
...
############################################################
## 8. Plots with explicit A–E labels
############################################################
...
pA <- ggplot(panelA_df, aes(x = Dataset, y = n_spots)) +
  ...
  theme_s20 +
  theme(axis.text.x = element_text(angle = 35, hjust = 1, vjust = 1, size = 11.5))

pB <- ggplot(panelB_df, aes(x = State, y = coverage, color = State)) +
  geom_errorbar(aes(ymin = low, ymax = high), width = 0.12, linewidth = 0.55) +
  geom_point(size = 2.2) +
  facet_wrap(~ Dataset, nrow = 1) +
  scale_color_manual(values = primary_state_colors, guide = "none") +
  ...
  theme_s20 +
  theme(axis.text.x = element_text(angle = 35, hjust = 1, vjust = 1, size = 10.75))
...
header <- ggplot() +
  annotate(
    ...
    hjust = 0.5,
    vjust = 1,
    label = "External spatial recurrence audits in melanoma spatial transcriptomics datasets",
    fontface = "bold",
    family = "sans",
    size = 5.25
  ) +
...
safe_ggsave(out_pdf, supp_s20, width = fig_width, height = fig_height, dpi = dpi_out)
...
write.csv(panelA_df, file.path(fig_dir, "Supplementary_Figure_S20_panelA_external_dataset_counts.csv"), row.names = FALSE)
write.csv(panelB_df, file.path(fig_dir, "Supplementary_Figure_S20_panelB_signature_gene_coverage.csv"), row.names = FALSE)
write.csv(panelC_df, file.path(fig_dir, "Supplementary_Figure_S20_panelC_spearman_correlation.csv"), row.names = FALSE)
write.csv(panelD_df, file.path(fig_dir, "Supplementary_Figure_S20_panelD_dual_high_enrichment.csv"), row.names = FALSE)
write.csv(panelE_df, file.path(fig_dir, "Supplementary_Figure_S20_panelE_bivariate_moran.csv"), row.names = FALSE)
