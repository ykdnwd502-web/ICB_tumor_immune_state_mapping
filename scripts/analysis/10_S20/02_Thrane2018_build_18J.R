...
##
##
## v1.0.1 public sequential compatibility fix:
##   Thrane raw count matrices encode first-column gene IDs as
##   "SYMBOL ENSG00000...".
##   The already-validated canonical v1.0.2 explicit compound-ID parser
##   is used here before the unchanged historical scoring logic.
##
## INPUT
##   data_external_spatial_18J/Thrane2018_legacyST/
##   (existing raw_zenodo ZIP and/or unpacked count matrices)
##
## OUTPUT
...
##   raw Thrane 2018 -> historical 18J outputs
##
## Scientific/statistical definitions are retained from the archived
## "18J-light Thrane 2018 melanoma legacy-ST external recurrence
## audit v4 FINAL".
## Automatic re-download is disabled for the current public rerun.
############################################################

# 18J-light Thrane 2018 melanoma legacy-ST external recurrence audit
# v4 FINAL: no AnnotationDbi::select conflict + ID-mapped version
#
# Purpose:
...
message2 <- function(...) {
  cat(paste0("[18J-Thrane2018-v4] ", paste0(..., collapse = ""), "\n"))
}
...
if (length(existing_unpacked_files) > 0) {
  ...
  if (!file.exists(ZENODO_ZIP) || !zip_ok(ZENODO_ZIP)) {
    stop(
      "No unpacked Thrane files were found and the local Zenodo ZIP is unavailable/unreadable: ",
      ZENODO_ZIP,
      call. = FALSE
    )
  }

  message2("No unpacked files found; unpacking the existing local ZIP...")
...
write_csv(file_inventory, "18J_Thrane2018_file_inventory.csv")
...
write_csv(ext_summary, "18J_Thrane2018_file_extension_summary.csv")
...
write_csv(audit_rows, "18J_Thrane2018_state_geneset_candidate_audit.csv")
write_csv(audit_wide, "18J_Thrane2018_state_geneset_candidate_audit_wide.csv")
...
if (is.na(best_file) || !best_file %in% names(parsed_list)) {
  stop(
    "No valid state gene-set file found. Check 18J_Thrane2018_state_geneset_candidate_audit_wide.csv.",
    call. = FALSE
  )
}
...
write_csv(gs_long, "18J_Thrane2018_state_genes_used_long.csv")
write_csv(state_counts, "18J_Thrane2018_state_genes_used_counts.csv")
...
write_csv(candidate_count_files, "18J_Thrane2018_candidate_count_files.csv")
...
write_csv(matrix_read_log, "18J_Thrane2018_matrix_read_log.csv")
write_csv(matrix_candidate_log, "18J_Thrane2018_matrix_candidate_orientation_log.csv")
write_csv(sample_input_log, "18J_Thrane2018_sample_input_log.csv")
write_csv(all_coverage, "18J_Thrane2018_gene_coverage.csv")
write_csv(all_scores, "18J_Thrane2018_spot_state_composition_scores.csv")

if (nrow(all_scores) == 0) {
  stop(
    "No usable expression matrix was detected. Check 18J_Thrane2018_matrix_candidate_orientation_log.csv. ",
...
write_csv(coverage_summary, "18J_Thrane2018_gene_coverage_summary.csv")
...
write_csv(df, "18J_Thrane2018_spot_scores_with_residualized_primary_pair.csv")
...
write_csv(cor_rows, "18J_Thrane2018_state_composition_correlation.csv")
...
write_csv(primary_cor, "18J_Thrane2018_primary_pair_raw_vs_residualized_correlation.csv")
...
write_csv(dual_rows, "18J_Thrane2018_primary_pair_dual_high_OR.csv")
...
write_csv(moran_rows, "18J_Thrane2018_primary_pair_Moran_descriptive.csv")
...
write_csv(key_readout, "18J_Thrane2018_key_readout.csv")
...
write_csv(include_decision, "18J_Thrane2018_inclusion_decision_helper.csv")
...
openxlsx::saveWorkbook(
  wb,
  file.path(OUT_DIR, "historical_Thrane2018_external_spatial_recurrence_audit.xlsx"),
  overwrite = TRUE
)
...
message2("18J-light Thrane 2018 legacy-ST audit completed.")
...
# ------------------------------------------------------------
expected_step02 <- PUBLIC_CORE_OUTPUTS

missing_step02 <- expected_step02[
  !file.exists(file.path(OUT_DIR, expected_step02))
]

if (length(missing_step02)) {
  stop(
    "STEP 02 finished but expected 18J file(s) are missing:\n",
    paste(missing_step02, collapse = "\n"),
    call. = FALSE
  )
}
...
cat(
  "\n============================================================\n",
  "STEP 02 PASS — Thrane2018 raw -> 18J\n",
