# S20 GSE250636 external spatial recurrence
#
# Purpose:
...
OUT_DIR <- file.path(CANONICAL_ROOT, GEO_ID)

dir.create(OUT_DIR, recursive = TRUE, showWarnings = FALSE)

if (!dir.exists(FLAT_DIR)) {
  stop(
    "GSE250636 flat Visium directory not found: ", FLAT_DIR,
    "\nExpected under current project architecture: ",
...
# Source-lock the exact four-state gene set used by the historical final analysis.
...
## Cross-dataset lock guard: the two final historical producers must use the same set.
...
STATE_GENESET_FILE <- HISTORICAL_GENESET_GSE
...
message2 <- function(...) {
  cat(paste0("[S20-GSE250636-CANONICAL-v1.0.1] ", paste0(..., collapse = ""), "\n"))
}
...
write_csv(sample_files, "S20_GSE250636_flat_sample_file_map.csv")
...
write_csv(audit_rows, "S20_GSE250636_state_geneset_candidate_audit.csv")
...
write_csv(audit_wide, "S20_GSE250636_state_geneset_candidate_audit_wide.csv")
...
if (is.na(best_file) || !best_file %in% names(parsed_list)) {
  stop(
    ...
    "Check S20_GSE250636_state_geneset_candidate_audit_wide.csv. ",
...
write_csv(gs_long, "S20_GSE250636_state_genes_used_long.csv")
write_csv(state_counts, "S20_GSE250636_state_genes_used_counts.csv")
...
write_csv(sample_input_log, "S20_GSE250636_sample_input_log.csv")
write_csv(all_coverage, "S20_GSE250636_gene_coverage.csv")
write_csv(all_scores, "S20_GSE250636_spot_state_composition_scores.csv")
...
write_csv(coverage_summary, "S20_GSE250636_gene_coverage_summary.csv")
...
if (nrow(critical_bad) > 0) {
  stop(
    ...
    "Check S20_GSE250636_gene_coverage_summary.csv and selected state gene-set file.",
    call. = FALSE
  )
}
...
write_csv(df, "S20_GSE250636_spot_scores_with_residualized_primary_pair.csv")
...
write_csv(cor_rows, "S20_GSE250636_state_composition_correlation.csv")
...
write_csv(primary_cor, "S20_GSE250636_primary_pair_raw_vs_residualized_correlation.csv")
...
write_csv(dual_rows, "S20_GSE250636_primary_pair_dual_high_OR.csv")
...
write_csv(moran_rows, "S20_GSE250636_primary_pair_Moran_descriptive.csv")
...
write_csv(key_readout, "S20_GSE250636_key_readout.csv")
...
openxlsx::saveWorkbook(
  wb,
  file.path(OUT_DIR, "S20_GSE250636_external_spatial_recurrence.xlsx"),
  overwrite = TRUE
)
...
message2("S20 GSE250636 CANONICAL v1.0 completed.")
